...
function setupDefaultView() { 
    ...
    var defaultView = { title: "Wine World", 
    view:  bodyView,
    };
...
function onMapButtonClick( event ) {
    var view = { title: "Carte",
             backLabel: (isTablet() ? "Retour" : " "),
...
//find the all wine producers within 15KM
function filterMarketsByGeo( latitude, longitude ) {
    var result = [];
    var startTime = new Date().getTime();// for testing speed load purpose only----------useless var
...
function distance( lat1, lon1, lat2, lon2 ) {
    ...
    var m = 6 / 1.609344; // Distance in miles----------useless var
    return d;
}
...
function showMarketDetails( item ) { 
    ...
    var view = { title: "Informations vigneron",
             backLabel: (isTablet() ? "Retour" : " "),
...
function onSearchButtonClick( event ) {
    ...
    var view = { title: "Résultats recherche",
             backLabel: (isTablet() ? "Retour" : " "),
...
function openExternalURL( url ) {
    ...
    var result=confirm("Vous allez quitter cette application.  Continuer?");
    if (result==true) {
